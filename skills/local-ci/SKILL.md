...
> **Note:** `ddev exec` always runs from the container's fixed working directory (`/var/www/html`), so a per-dir `cd` before invoking a PHP check doesn't reach the container on its own. local-ci works around this for a PHP dir that sits *nested* under a different project's DDEV root (e.g. a SilverStripe module developed as a live git checkout inside that project's `vendor/` tree, rather than a plain composer-installed package): it walks up from the scanned dir looking for an ancestor `.ddev/config.yaml`, and if found, routes PHP/composer checks through `ddev exec -d <relpath-from-root>` (and `ddev exec -d <relpath> composer` for composer commands) instead of falling back to a bare-metal toolchain. This does not by itself prevent a nested module's own `composer.json` from pulling in a `require-dev` package whose post-install script assumes it's a project root (e.g. a test-scaffold recipe) - that risk exists regardless of ddev vs bare metal, and local-ci flags it with a WARN on the module's first install rather than silently letting it happen. Pass the nested dir as an explicit `DIR` argument to scan it (see Options above); scanning only the project root does not also check its nested `vendor/` live checkouts.
