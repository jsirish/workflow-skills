...
- A project with no recognised configs reports "nothing to run" rather than erroring. A project that *does* have a recognised config (composer.json, package.json, a Python marker file, `*.sh` sources, or `.local-ci.json`) but where a driver still records nothing gets a different message naming that as a local-ci bug, not silence that reads as "no CI surface here."
