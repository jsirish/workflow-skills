...
# VENV_BIN lookup (an active $VIRTUAL_ENV, else ./.venv, else ./venv) that
# takes priority over both the global binary and a stray older/newer globally
# installed one.
#
...
EOF
  ...
  out="$(cd "$work" && PATH="$bin" bash "$LOCAL_CI" --no-fix "$fixture" 2>&1)"
...
$out" ;;
  ...
  out="$(cd "$work" && PATH="$bin" bash "$LOCAL_CI" --no-fix --strict "$fixture" 2>&1)"
  case "$out" in
